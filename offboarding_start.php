...
// Solo admin e editor possono avviare offboarding
if (!in_array($_SESSION['crm_user']['crm_role'], ['admin', 'editor'])) {
    header('Location: agenzie.php');
    exit;
}

$pageTitle = "Avvia Offboarding - CRM Coldwell Banker";
$pdo = getDB();
...
if (!$agency || $agency['status'] !== 'Closing') {
    header('Location: agenzie.php');
    exit;
}

// Verifica se offboarding già attivo
...
// Gestione POST - avvia offboarding
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    
    // Crea offboarding
    $stmt = $pdo->prepare("INSERT INTO offboardings (agency_id, started_by) VALUES (:agency_id, :started_by)");
    $stmt->execute([
        'agency_id' => $agency_id,
        'started_by' => $_SESSION['crm_user']['id'] ?? 0
    ]);
    $offboarding_id = $pdo->lastInsertId();
    
    // Copia task dal template con responsabili modificati
    $stmt = $pdo->prepare("SELECT * FROM offboarding_template WHERE is_active = 1 ORDER BY phase, task_order");
    $stmt->execute();
    $template_tasks = $stmt->fetchAll();
    
    $insert_task = $pdo->prepare("INSERT INTO offboarding_tasks (offboarding_id, phase, task_name, task_order, assignee) VALUES (?, ?, ?, ?, ?)");
    ...
    $pdo->prepare("UPDATE agencies SET status = 'In Offboarding' WHERE id = ?")->execute([$agency_id]);
    
    // Log
    $userId = $_SESSION['crm_user']['id'] ?? null;
    if ($userId) {
        logAudit($pdo, $userId, $_SESSION['crm_user']['email'] ?? 'unknown', 'agencies', $agency_id, 'UPDATE', [
            'status' => ['old' => 'Closing', 'new' => 'In Offboarding']
        ]);
    }
...
$stmt = $pdo->query("SELECT * FROM offboarding_template WHERE is_active = 1 ORDER BY phase, task_order");
$template_tasks = $stmt->fetchAll();
...
.form-card{background:white;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,.1);border-left:6px solid #EF4444;padding:2rem;margin-bottom:2rem}
...
.phase-header{font-size:1.1rem;font-weight:600;color:#EF4444;margin-bottom:1.5rem;display:flex;align-items:center;gap:.5rem}
...
.btn-start{background:#EF4444;color:white;border:none;padding:.75rem 1.5rem;border-radius:8px;cursor:pointer;font-weight:600;transition:background .2s;display:inline-flex;align-items:center;gap:.5rem}
.btn-start:hover{background:#DC2626}
.info-box{background:#FEE2E2;border-left:4px solid #EF4444;padding:1rem;border-radius:8px;margin-bottom:2rem}
.info-box p{margin:0;color:#991B1B;font-size:.95rem}
</style>

<div class="page-header">
<div>
<h1 class="page-title">📤 Avvia Offboarding</h1>
...
<div class="info-box">
<p><strong>ℹ️ Conferma responsabili:</strong> I task sono preassegnati secondo il template. Puoi modificare i responsabili prima di avviare l'offboarding.</p>
</div>
...
<button type="submit" class="btn-start">📤 Avvia Offboarding</button>
</div>
