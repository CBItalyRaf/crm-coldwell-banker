...
// Carica onboarding attivo
$stmt = $pdo->prepare("SELECT * FROM onboardings WHERE agency_id = :agency_id AND status = 'active'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'complete_task') {
        ...
        $stmt = $pdo->prepare("UPDATE onboarding_tasks SET is_completed = 1, completed_by = ?, completed_at = NOW(), completion_notes = ? WHERE id = ?");
        $stmt->execute([$_SESSION['crm_user']['id'] ?? 0, $notes, $task_id]);
        
        // Log
        $stmt = $pdo->prepare("SELECT task_name FROM onboarding_tasks WHERE id = ?");
        $stmt->execute([$task_id]);
        $task = $stmt->fetch();
        
        logAudit($pdo, $_SESSION['crm_user']['id'] ?? 0, $_SESSION['crm_user']['email'], 'onboarding_tasks', $task_id, 'UPDATE', [
            'task_name' => ['old' => $task['task_name'], 'new' => $task['task_name'] . ' - COMPLETATO']
        ]);
        
        header("Location: onboarding_detail.php?agency_id=$agency_id&success=task_completed");
        exit;
    }
    ...
    if ($_POST['action'] === 'uncomplete_task') {
        ...
        $stmt = $pdo->prepare("UPDATE onboarding_tasks SET is_completed = 0, completed_by = NULL, completed_at = NULL, completion_notes = NULL WHERE id = ?");
        $stmt->execute([$task_id]);
    ...
    if ($_POST['action'] === 'close_onboarding' && in_array($_SESSION['crm_user']['crm_role'], ['admin', 'editor'])) {
        
        // Aggiorna onboarding
        $pdo->prepare("UPDATE onboardings SET status = 'completed', completed_at = NOW() WHERE id = ?")->execute([$onboarding['id']]);
        
        // Aggiorna status agenzia
        $pdo->prepare("UPDATE agencies SET status = 'Active' WHERE id = ?")->execute([$agency_id]);
        
        // Log
        logAudit($pdo, $_SESSION['crm_user']['id'] ?? 0, $_SESSION['crm_user']['email'], 'agencies', $agency_id, 'UPDATE', [
            'status' => ['old' => 'In Onboarding', 'new' => 'Active']
        ]);
        
        header("Location: agenzia_detail.php?code=" . urlencode($agency['code']) . "&success=onboarding_completed");
        exit;
    }
}

// Carica task
$stmt = $pdo->prepare("SELECT * FROM onboarding_tasks WHERE onboarding_id = :onboarding_id ORDER BY phase, task_order");
$stmt->execute(['onboarding_id' => $onboarding['id']]);
...
.btn-close{background:#EF4444;color:white;border:none;padding:.75rem 1.5rem;border-radius:8px;cursor:pointer;font-weight:600;transition:background .2s}
...
.phase-header{background:var(--cb-blue);color:white;padding:1rem 1.5rem;font-size:1.1rem;font-weight:600;display:flex;align-items:center;gap:.5rem}
...
<div>
<h1 class="page-title">🚀 Onboarding in Corso</h1>
...
<form method="POST" style="margin:0" onsubmit="return confirm('Sei sicuro di voler chiudere questo onboarding? L\'agenzia passerà a status Active.')">
<input type="hidden" name="action" value="close_onboarding">
<button type="submit" class="btn-close" <?= $progress < 100 ? 'disabled title="Completa tutti i task prima di chiudere"' : '' ?>>✓ Chiudi Onboarding</button>
</form>
<?php endif; ?>
