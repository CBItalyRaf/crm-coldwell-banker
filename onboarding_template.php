...
$pageTitle = "Template Onboarding - CRM Coldwell Banker";
$pdo = getDB();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] === 'add_task') {
            $stmt = $pdo->prepare("INSERT INTO onboarding_template (phase, task_name, task_order, default_assignee) VALUES (?, ?, ?, ?)");
            $stmt->execute([
                $_POST['phase'],
                $_POST['task_name'],
                $_POST['task_order'],
        ...
        if ($_POST['action'] === 'update_task') {
            $stmt = $pdo->prepare("UPDATE onboarding_template SET task_name = ?, default_assignee = ?, task_order = ? WHERE id = ?");
            $stmt->execute([
                $_POST['task_name'],
                $_POST['default_assignee'],
                $_POST['task_order'],
        ...
        if ($_POST['action'] === 'delete_task') {
            $pdo->prepare("UPDATE onboarding_template SET is_active = 0 WHERE id = ?")->execute([$_POST['task_id']]);
        }
        
        if ($_POST['action'] === 'restore_task') {
            $pdo->prepare("UPDATE onboarding_template SET is_active = 1 WHERE id = ?")->execute([$_POST['task_id']]);
        }
        
        header('Location: onboarding_template.php?success=1');
        exit;
    }
}

// Carica task
$stmt = $pdo->query("SELECT * FROM onboarding_template ORDER BY phase, task_order");
...
.phase-header{background:var(--cb-blue);color:white;padding:1rem 1.5rem;font-size:1.1rem;font-weight:600}
...
<h1 class="page-title">⚙️ Gestione Template Onboarding</h1>
...
<option value="1">Fase 1: Fino alla firma</option>
<option value="2">Fase 2: Pre apertura</option>
<option value="3">Fase 3: Apertura</option>
</select>
</div>
<div class="form-field">
